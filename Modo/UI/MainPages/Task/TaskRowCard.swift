...
struct TaskRowCard: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            if !isFutureDate {
                Button {
                    ...
                    ZStack {
                        Circle()
                            .fill(isDone ? emphasis : Color(.systemBackground))
                            .frame(width: 22, height: 22)
                            .overlay(
                                Circle().stroke(Color(UIColor.separator), lineWidth: isDone ? 0 : 1)
                            )
                    ...
                    GeometryReader { geometry in
                        ZStack(alignment: .leading) {
                            Text(title)
                                ...
                                .foregroundColor(isDone ? emphasis : .primary)
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .stroke(isDone ? emphasis.opacity(0.25) : Color(UIColor.separator), lineWidth: 1)
        )
